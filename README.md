...
- 🔐 **安全管理**：支持SQLite / PostgreSQL / MySQL存储，用户密码采用PBKDF2+SHA-256哈希，邮箱凭据与附件内容在数据库中对称加密
...
4. **数据库**：SQLite / PostgreSQL / MySQL 关系型数据库用于数据存储（可配置）
5. **邮件处理引擎**：处理邮件收取、解析和存储的核心模块

## ⚙️ 数据库配置

后端默认使用SQLite存储数据，您可以通过环境变量切换到PostgreSQL或MySQL：

- `DB_TYPE`：数据库类型，可选值为`sqlite`（默认）、`postgresql`、`mysql`
- `DB_URL`：可选，完整的数据库连接字符串（适用于PostgreSQL）
- `DB_HOST` / `DB_PORT` / `DB_NAME` / `DB_USER` / `DB_PASSWORD`：当未提供`DB_URL`时用于构建PostgreSQL或MySQL连接
- `DB_ENCRYPTION_KEY`：可选的32字节URL安全Base64密钥，若未设置则程序会在`backend/data/db_encryption.key`中自动生成并持久化

邮件账户密码、OAuth凭据以及附件内容都会在写入数据库前自动加密，并在读取时透明解密，以确保敏感数据的安全性。
...
5. 邮箱账号和密码等敏感信息会以加密形式存储在配置的关系型数据库中，请妥善保管数据库访问凭据与加密密钥，防止数据泄露。
