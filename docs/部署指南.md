...
> 官方容器镜像会同时发布到两个仓库：
> - Docker Hub：`luofengyuan/firemail`
> - GitHub Container Registry：`ghcr.io/<github用户名或组织>/<仓库名>`
>
> 项目根目录下的 `docker-build.yml` GitHub Actions 工作流会在推送代码或发布新标签时自动构建多架构镜像，并分别推送到 Docker Hub 与 GHCR。要启用自动发布功能，请在仓库 Secrets 中配置以下凭据：
> - `DOCKER_HUB_USERNAME`：Docker Hub 用户名
> - `DOCKER_HUB_TOKEN`：具备推送权限的 Docker Hub 访问令牌
> - `GH_TOKEN`：具备 `write:packages` 权限的 GitHub Personal Access Token，用于推送到 GHCR
>
> ⚠️ 如果未配置 `GH_TOKEN`，或令牌缺少 `write:packages` 权限，工作流会跳过 GHCR 登录与推送，仅发布 Docker Hub 镜像，并在日志中给出警告提示。
>
> 示例命令中使用的`IMAGE_NAME`可以替换为上述任意镜像（包含具体标签，如`:latest`或特定版本标签）。

### 基本部署

```bash
# 选择镜像源
IMAGE_NAME=ghcr.io/<github用户名或组织>/<仓库名>:latest
# 如果要使用 Docker Hub 镜像，请改为：
# IMAGE_NAME=luofengyuan/firemail:latest
...
docker run -d \
  ...
  -e DB_ENCRYPTION_KEY=base64url-32bytes-key \
...
> **变量说明**
>
> - `DB_TYPE`：数据库类型，可选 `sqlite` / `postgresql` / `mysql`
> - `DB_HOST`、`DB_PORT`、`DB_NAME`、`DB_USER`、`DB_PASSWORD`：数据库连接信息，使用 `sqlite` 时无需设置
> - `DB_URL`：可选，完整连接字符串，设置后优先生效
> - `DB_ENCRYPTION_KEY`：32 字节 Base64URL 编码的对称加密密钥，未设置时程序会自动生成并写入 `backend/data/db_encryption.key`
...
services:
  FireMail:
    image: ghcr.io/<github用户名或组织>/<仓库名>:latest  # 使用 Docker Hub 时改为 luofengyuan/firemail:latest
    ...
    environment:
      ...
      - DB_ENCRYPTION_KEY=base64url-32bytes-key
```
...
export DB_ENCRYPTION_KEY=base64url-32bytes-key

# Windows
set JWT_SECRET_KEY=your_secret_key_here
set FLASK_PORT=5000
set WS_PORT=8765
set DB_TYPE=sqlite  # 或 postgresql/mysql
rem set DB_URL=postgresql+psycopg://username:password@db-host:5432/firemail
rem set DB_HOST=127.0.0.1
rem set DB_PORT=3306
rem set DB_NAME=firemail
rem set DB_USER=firemail
rem set DB_PASSWORD=change_me
set DB_ENCRYPTION_KEY=base64url-32bytes-key
```
